...
const (
	...
	AccessToken1      = "ACCESS TOKEN"
	AccessTokenSecret = "ACCESS TOKEN SECRET"
)

//RetweetCount : Struct to bind retweet count
type RetweetCount struct {
...
func main() {
	...
	accessToken := &AccessToken{Token: AccessToken1, Secret: AccessTokenSecret}
	fmt.Scanf("%s", &str)
	twitterEndPoint := "https://api.twitter.com/1.1/statuses/retweets/" + str + ".json"
	resp, err := consumer.Get(twitterEndPoint, nil, accessToken)
	if err != nil {
		panic(err)
	}
	...
	key := make([]RetweetCount, 0)
	json.Unmarshal(respBody, &key)
	fmt.Println(key[0].Retweets)
}
